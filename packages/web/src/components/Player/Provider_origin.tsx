...
export const PlayerProvider = (props: PropsWithChildren<{ adapter: NativePlayerAdapter }>) => {
  ...
  const [nextManual, setNextManual] = useState<boolean>(false);
  const [prevManual, setPrevManual] = useState<boolean>(false);
  ...
  const playNextManual = useCallback(() => {
    if (playlist.length || repeat === 'all') {
      if (currentPlayable) {
        played.unshift(currentPlayable);
        setPlayed(played);
  ...
  const playPrevManual = useCallback(() => {
    if (currentPlayable && playedTime > PREV_THRESHOLD) {
      controlRef.current?.play(currentPlayable.sources || [], true);
    } else {
      if (played.length) controlRef.current?.play(played[0].sources || [], true);
  ...
  const player = useMemo<PlayerControl>(() => {
    return {
      ...
      addPlayable(playable) {
        // let collectionAfterAdd = {
        //   id: currentCollection.id,
        //   name: currentCollection.name,
        //   playables: [...currentCollection.playables,playable],
        // } as Collection
        // setCurrentCollection(collectionAfterAdd);
      ...
      addPlayableNext(playable) {
        ...
        //   id: currentCollection.id,
        //   name: currentCollection.name,
        //   playables: [playable,...currentCollection.playables],
        // } as Collection
        // setCurrentCollection(collectionAfterAdd);
